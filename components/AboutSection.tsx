...
import {
  ...
  BookOpen, Layers, Zap, Shield, Network, X,
} from "lucide-react";
import { seedAboutArticles, seedNetworkStats, type CmsAboutArticle, type CmsNetworkStat } from "@/lib/cms-content";
import { ViewCount } from "@/components/ViewCounts";

interface ModalContent { title: string; subtitle?: string; body: string; }
interface Suggestion { id: string; label: string; index: number; page: number; }
...
function coverImage(article: CmsAboutArticle) {
  return /^https?:\/\//i.test(article.coverImageUrl || "") ? article.coverImageUrl! : null;
}

function buildSuggestions(query: string, tab: string, aboutArticles: CmsAboutArticle[], networkStats: CmsNetworkStat[]): Suggestion[] {
  ...
  if (tab === "magazine") {
    return aboutArticles
      ...
      .slice(0, 6)
      .map(({ a, i }) => ({ id: `mag-${i}`, label: a.title, index: i, page: Math.ceil((i + 1) / CARDS_PER_PAGE) }));
  }
  return networkStats
    .map((s, i) => ({ s, i }))
    .filter(({ s }) => s.label.toLowerCase().includes(q) || s.desc.toLowerCase().includes(q))
    .slice(0, 6)
    .map(({ s, i }) => ({ id: `net-${i}`, label: s.label, index: i, page: 1 }));
}
...
export default function AboutSection() {
  ...
  const [aboutArticles, setAboutArticles] = useState<CmsAboutArticle[]>(seedAboutArticles);
  const [networkStats, setNetworkStats] = useState<CmsNetworkStat[]>(seedNetworkStats);

  const searchWrap = useRef<HTMLDivElement>(null);
  const cardRefs = useRef<Map<string, HTMLDivElement>>(new Map());

  useEffect(() => {
    fetch("/api/cms/aboutArticles", { cache: "no-store" })
      .then((response) => response.json())
      .then((data) => Array.isArray(data) && setAboutArticles(data))
      .catch(() => setAboutArticles(seedAboutArticles));

    fetch("/api/cms/networkStats", { cache: "no-store" })
      .then((response) => response.json())
      .then((data) => Array.isArray(data) && setNetworkStats(data))
      .catch(() => setNetworkStats(seedNetworkStats));
  }, []);

  useEffect(() => { setPage(1); setSearch(""); setSuggestions([]); setShowSug(false); }, [tab]);

  useEffect(() => {
    const s = buildSuggestions(search, tab, aboutArticles, networkStats);
    setSuggestions(s);
    setShowSug(search.trim().length >= 2 && s.length > 0);
  }, [search, tab, aboutArticles, networkStats]);
  ...
  const filteredArticles = aboutArticles.filter(a =>
    !q || a.title.toLowerCase().includes(q) || a.category.toLowerCase().includes(q) || a.body.toLowerCase().includes(q)
  );
  const filteredStats = networkStats.filter(s =>
    !q || s.label.toLowerCase().includes(q) || s.desc.toLowerCase().includes(q) || s.value.toLowerCase().includes(q)
  );

  const totalPages = tab === "magazine" ? Math.ceil(filteredArticles.length / CARDS_PER_PAGE) : 1;
  const pageArticles = filteredArticles.slice((page - 1) * CARDS_PER_PAGE, page * CARDS_PER_PAGE);

  const sidebarItems = (tab === "magazine" ? aboutArticles : networkStats).map((item, i) => ({
    id: `${tab}-card-${i}`,
    label: tab === "magazine" ? (item as typeof aboutArticles[0]).title : (item as typeof networkStats[0]).label,
    page: tab === "magazine" ? Math.ceil((i + 1) / CARDS_PER_PAGE) : 1,
  }));
    ...
    <div className="relative min-h-screen" style={{ zIndex: 2 }}>
      ...
      <div style={{ borderBottom: "1px solid var(--border)", background: "var(--muted)" }}>
        <div className="px-8 md:px-14 flex">
          {[
            { id: "magazine", label: "Quai Magazine", Icon: BookOpen },
            { id: "network", label: "Quai Network", Icon: Network },
          ].map(({ id, label, Icon }) => (
            <button
              key={id}
              onClick={() => router.push(`/about?tab=${id}`)}
      ...
      <div className="flex" style={{ minHeight: "calc(100vh - 220px)" }}>
        ...
        >
          <p className="text-sm font-mono tracking-[0.15em] mb-3" style={{ color: "rgba(255,107,0,0.65)" }}>
            {tab === "magazine" ? "MAGAZINE" : "NETWORK INFO"}
          </p>
          <div className="space-y-2.5 mb-6">
            {tab === "magazine" ? (
              <>
                <div className="flex items-center gap-2.5">
                  <BookOpen size={10} style={{ color: "rgba(255,107,0,0.55)", flexShrink: 0 }} />
                  <span className="text-sm" style={{ color: "var(--tx-body)", fontFamily: "Inter, sans-serif" }}>6 Articles</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <Layers size={10} style={{ color: "rgba(255,107,0,0.55)", flexShrink: 0 }} />
                  <span className="text-sm" style={{ color: "var(--tx-body)", fontFamily: "Inter, sans-serif" }}>Multiple Categories</span>
                </div>
              </>
            ) : (
              <>
                <div className="flex items-center gap-2.5">
                  <Zap size={10} style={{ color: "rgba(255,107,0,0.55)", flexShrink: 0 }} />
                  <span className="text-sm" style={{ color: "var(--tx-body)", fontFamily: "Inter, sans-serif" }}>50,000+ TPS</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <Shield size={10} style={{ color: "rgba(255,107,0,0.55)", flexShrink: 0 }} />
                  <span className="text-sm" style={{ color: "var(--tx-body)", fontFamily: "Inter, sans-serif" }}>Proof-of-Work</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <Network size={10} style={{ color: "rgba(255,107,0,0.55)", flexShrink: 0 }} />
                  <span className="text-sm" style={{ color: "var(--tx-body)", fontFamily: "Inter, sans-serif" }}>13 Chains</span>
                </div>
              </>
            )}
          </div>
        ...
        <main className="flex-1 min-w-0 py-8 px-6 md:px-10">
          ...
          <div className="max-w-2xl mb-7" ref={searchWrap}>
            <div className="relative">
              ...
              <input
                ...
                placeholder={tab === "magazine" ? "Search articles, categories…" : "Search network stats…"}
                ...
                {showSug && (
                  ...
                  >
                    {suggestions.map((s, si) => (
                      ...
                      >
                        <span className="text-sm font-mono px-1.5 py-0.5 rounded flex-shrink-0" style={{ background: "rgba(255,107,0,0.12)", color: "var(--accent-text)" }}>
                          {tab === "magazine" ? "ART" : "STAT"}
                        </span>
                        <span className="truncate text-sm" style={{ color: "var(--tx-mid)", fontFamily: "Inter, sans-serif" }}>{s.label}</span>
                      </button>
                    ))}
                  </motion.div>
                )}
          ...
          <div className="flex items-center max-w-2xl mb-5">
            <span className="text-sm" style={{ color: "var(--tx-dim)", fontFamily: "Inter, sans-serif" }}>
              {tab === "magazine" ? `${filteredArticles.length} articles` : `${filteredStats.length} stats`}
              {search ? ` · "${search}"` : ""}
            </span>
          </div>
              ...
              {pageArticles.map((a, i) => {
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => router.push(`/about/article/${a.id}`)}
                  ...
                  >
                    {coverImage(a) && <div className="h-40" style={{ background: "var(--muted)" }}><img src={coverImage(a)!} alt="" className="h-full w-full object-cover" /></div>}
                    {/* Header stripe */}
                    ...
                    <div className="px-5 pt-4 pb-5">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-3"><p className="text-sm font-mono" style={{ color: "var(--tx-faint)" }}>{a.date}</p><ViewCount count={a.viewCount} /></div>
                    </div>
                  </motion.div>
                );
              })}
          ...
          {tab === "network" && (
            <>
                ...
                {filteredStats.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="font-mono text-sm tracking-wider mb-1" style={{ color: "rgba(255,107,0,0.65)" }}>{s.label}</div>
                      <div className="text-[32px] font-bold mb-2" style={{ fontFamily: "Rajdhani, sans-serif", color: "var(--tx-high)" }}>{s.value}</div>
                      <p style={{ fontSize: "14px", lineHeight: "1.65", fontFamily: "Inter, sans-serif", color: "var(--tx-dim)", ...clampStyle }}>{s.desc}</p>
                      <button
                        onClick={() => setModal({ title: s.label, subtitle: s.value, body: s.desc })}
                        className="mt-2.5 flex items-center gap-1 text-sm font-medium hover:opacity-70 transition-opacity"
                        style={{ color: "var(--accent-text)", fontFamily: "Inter, sans-serif" }}
                      >
                        <ChevronDown size={11} /> See more
              ...
              <motion.div
                ...
                style={{ background: "var(--glass-bg)", border: "1px solid var(--border)", boxShadow: "0 2px 12px rgba(0,0,0,0.10)" }}
              >
                <h2 className="text-[24px] font-bold mb-4" style={{ fontFamily: "Rajdhani, sans-serif", color: "var(--tx-high)" }}>The Architecture</h2>
                <p className="text-sm leading-relaxed mb-3" style={{ fontFamily: "Inter, sans-serif", color: "var(--tx-body)" }}>
                  Quai Network solves the blockchain trilemma through a hierarchical sharding model. The Prime chain coordinates 3 Region chains, each of which coordinates 3 Zone chains — creating 13 parallel blockchains that merge-mine simultaneously.
                </p>
                <p className="text-sm leading-relaxed" style={{ fontFamily: "Inter, sans-serif", color: "var(--tx-body)" }}>
                  Transactions within a Zone chain confirm in seconds. Cross-zone transactions propagate through the hierarchy and settle within seconds to minutes — layer-2 throughput with layer-1 security guarantees.
                </p>
              </motion.div>
            </>
          )}
